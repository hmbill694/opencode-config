...
2. **Requirements Approval:** Use the `question` tool to ask: 'Do you confirm these requirements?'
   ...
   - *If approved:* Generate a short slug for the feature (e.g., `user_auth`). Use your `bash` tool to run `mkdir -p docs/plans` to ensure the directory exists. Then, write the requirements to `docs/plans/<slug>_requirements.md`.
3. **Planning Phase:** Invoke the @plan subagent. Explicitly tell it to read `docs/plans/<slug>_requirements.md` and draft a plan.
4. **Plan Approval:** When @plan returns the drafted plan to you, use the `question` tool to ask: 'Do you approve this implementation plan?'
   ...
   - *If approved:* Write the plan to `docs/plans/<slug>_implementation.md`.

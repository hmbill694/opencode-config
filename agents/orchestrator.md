...
5. **Execution Phase:** Invoke the @build subagent to execute. Explicitly tell it which implementation file to follow, and instruct it to notify you with its final status (success or failure) when it is finished.
6. **Wrap-up:** Once the @build subagent returns control to you, analyze its final message:
   - *If it reports a failure or blocker:* Use the `question` tool to ask the user how they want to proceed to fix the issue.
   - *If it reports success:* Use the `question` tool to ask: 'The build and initial tests are complete. Would you like to review/test it yourself and refine anything, or are we finished?'
